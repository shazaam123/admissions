  ...
  <div class="container">
    <div class="col s12"><input id="searchbar_toggle" type="checkbox" />
      <div id="searchbar" class="row">
        <div class="col s12 l2">
          <select id="search_col" @change="searchChange()">
            <option value="Enrollment.id" data-type="number">Enrollment Id</option>
            <option value="Enrollment.student" data-type="number">Enrollment Student</option>
            <option value="Enrollment.course" data-type="number">Enrollment Course</option>
            <option value="Enrollment.enrollment_date" data-type="date">Enrollment Enrollment Date</option>
            <option value="Enrollment.grade">Enrollment Grade</option>
          </select>
        </div>
        <div class="col s12 l2">
      ...
      <table class="striped highlight">
        <thead>
          <tr>
            <th :class="getSortClass('Enrollment.id','asc')">
              <router-link :to="getLink('sort','Enrollment.id','asc')">Id</router-link>
            </th>
            <th :class="getSortClass('Enrollment.student')">
              <router-link :to="getLink('sort','Enrollment.student')">Student</router-link>
            </th>
            <th :class="getSortClass('Enrollment.course')">
              <router-link :to="getLink('sort','Enrollment.course')">Course</router-link>
            </th>
            <th :class="getSortClass('Enrollment.enrollment_date') + ' hide-on-small-only'">
              <router-link :to="getLink('sort','Enrollment.enrollment_date')">Enrollment Date</router-link>
            </th>
            <th :class="getSortClass('Enrollment.grade') + ' hide-on-small-only'">
              <router-link :to="getLink('sort','Enrollment.grade')">Grade</router-link>
            </th>
            <th>Actions</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="enrollment in enrollments" :key="enrollment">
            <td class="center-align">{{enrollment.id}}</td>
            ...
            <td class="center-align">
              <router-link class="btn-small grey" :to="`/enrollment/${enrollment.id}`" title="View"><i class="mdi mdi-eye"></i></router-link>
              <router-link class="btn-small" :to="`/enrollment/edit/${enrollment.id}`" title="Edit"><i class="mdi mdi-pencil"></i></router-link>
              <router-link class="btn-small red" :to="`/enrollment/delete/${enrollment.id}`" title="Delete"><i class="mdi mdi-close-thick"></i></router-link>
            </td>
          </tr>
        </tbody>
      </table>
      ...
      <router-link class="btn-small" to="/enrollment/create">Create</router-link>
    </div>
