  ...
  <div class="container">
    <div class="col s12"><input id="searchbar_toggle" type="checkbox" />
      <div id="searchbar" class="row">
        <div class="col s12 l2">
          <select id="search_col" @change="searchChange()">
            <option value="Faculty.id" data-type="number">Faculty Id</option>
            <option value="Faculty.facultyName">Faculty Faculty Name</option>
            <option value="Faculty.email">Faculty Email</option>
            <option value="Faculty.phone">Faculty Phone</option>
          </select>
        </div>
        <div class="col s12 l2">
      ...
      <table class="striped highlight">
        <thead>
          <tr>
            <th :class="getSortClass('Faculty.id','asc')">
              <router-link :to="getLink('sort','Faculty.id','asc')">Id</router-link>
            </th>
            <th :class="getSortClass('Faculty.facultyName')">
              <router-link :to="getLink('sort','Faculty.facultyName')">Faculty Name</router-link>
            </th>
            <th :class="getSortClass('Faculty.email')">
              <router-link :to="getLink('sort','Faculty.email')">Email</router-link>
            </th>
            <th :class="getSortClass('Faculty.phone') + ' hide-on-small-only'">
              <router-link :to="getLink('sort','Faculty.phone')">Phone</router-link>
            </th>
            <th>Actions</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="faculty in faculties" :key="faculty">
            <td class="center-align">{{faculty.id}}</td>
            <td>{{faculty.facultyName}}</td>
            <td>{{faculty.email}}</td>
            <td class="hide-on-small-only">{{faculty.phone}}</td>
            <td class="center-align">
              <router-link class="btn-small grey" :to="`/faculty/${faculty.id}`" title="View"><i class="mdi mdi-eye"></i></router-link>
              <router-link class="btn-small" :to="`/faculty/edit/${faculty.id}`" title="Edit"><i class="mdi mdi-pencil"></i></router-link>
              <router-link class="btn-small red" :to="`/faculty/delete/${faculty.id}`" title="Delete"><i class="mdi mdi-close-thick"></i></router-link>
            </td>
          </tr>
        </tbody>
      </table>
      ...
      <router-link class="btn-small" to="/faculty/create">Create</router-link>
    </div>
